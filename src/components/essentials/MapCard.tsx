...
const MapCard = ({
  ...
  return (
    <>
      <div
        className={`bg-${backgroundColor} rounded-xl border border-${borderColor}`}
      >
        ...
        >
          <iframe
            ...
            className={`w-full h-full border-0 opacity-${opacity} p-2`}
            loading="lazy"
            allowFullScreen
          ></iframe>
        </div>
        {isBtnAvailable && (
          <div className={`text-center mt-2 bg-[${footerColor}]`}>
            <button
              className={`bg-[${btnColor}] text-[${textColor}]  p-2 pl-4 pr-4 mb-2 rounded-lg cursor-pointer hover:scale-105`}
            >
              <a
