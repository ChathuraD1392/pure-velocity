    ...
    <>
      ...
      >
        <div className={`text-[${paraColor}] text-center`}>
          <h1 className={`text-2xl font-medium ${title ? "pt-2 pb-1" : ""}`}>
            {title}
          </h1>
          <p className={`font-light text-sm`}>{mobile}</p>
          <p className={`font-light text-sm ${address ? "pb-1" : ""}`}>
            {address}
          </p>
        </div>
        <div
        ...
        >
          <iframe
            ...
            className={`w-full h-full border-0 rounded-lg opacity-${opacity} shadow-xs`}
            loading="lazy"
            allowFullScreen
          ></iframe>
        </div>
